                ...
                <div class="col-7">
                    <div class="row items-push">
                        <div class="col-12 d-flex justify-content-between">
                            <span class="fs-5 fw-semibold"> Dataset: {{ $task->dataset->name }} </span>
                            @php
                                $url = url()->previous();
                                if (url()->previous() == url()->current()) {
                                    $url = route('dca_ma.tasks', [$application->id]);
                                }
                            @endphp
                            <a class="btn btn-sm btn-primary" href="{{ $url }}">Back</a>
                        </div>
                        <div class="col-12">
                            ...
                            <div class="d-flex align-items-center justify-content-between p-0">
                                ...
                                <div>
                                    ...
                                    <a class="btn btn-sm btn-primary @disabled(is_null($parameters['prev_id']))"
                                        href="{{ route('dca_ma.manual_labelling', [$application->id, $task->id, $parameters['prev_id']]) }}">
                                        <i class="fa fa-caret-left"></i> Prev
                                    </a>
                                    <a class="btn btn-sm btn-primary @disabled(is_null($parameters['next_id']))"
                                        href="{{ route('dca_ma.manual_labelling', [$application->id, $task->id, $parameters['next_id']]) }}">
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $('#submit').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('dca_ma.update_manual_labelling', [$application->id, $task->id, $reference->id]) }}",
                type: 'POST',
                DataType: 'json',
                headers: {
                    'X-CSRF-TOKEN': token,
    ...
    function initResultsTable() {
        const columns = [{
            ...
            {
                data: 'ai_result',
                name: 'dca_ma_results.ai_result',
            },
            {
                data: 'ai_code',
                name: 'dca_ma_results.ai_code',
            },
            {
                data: 'label_result',
                name: 'dca_ma_results.label_result',
            },
            {
                data: 'label_code',
                name: 'dca_ma_results.label_code',
            },
            {
                data: 'remarks',
                name: 'remarks',
        ...
        const filterColumns = {
            "inputs": ["ID", "AI Result", "AI Code", "Label Result", "Label Code", "Remarks"],

        };
        const ajax_url = "{{ route('dca_ma.results', [$application->id, $reference->id]) }}";
