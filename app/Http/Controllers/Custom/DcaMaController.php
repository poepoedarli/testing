...
use App\Models\Custom\DcaMaCorrections;
...
class DcaMaController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('actions', function ($row) {
                    ...
                    if ($row->completed) {
                        $buttons .= FormControls::anchor_control(route('dca_ma.manual_labelling', [$row->application->id, $row->id]), 'Manual Labelling', 'btn btn-sm btn-secondary me-1', 'fa-edit');
                        // show after manual labelling is done
                        $buttons .= FormControls::anchor_control(route('dca_ma.report', [$row->application->id, $row->id]), 'Report', 'btn btn-sm btn-secondary me-1', 'fa-clipboard');
                    }
                    return $buttons;
    ...
    public function results(HttpRequest $request, string $id, string $ref_id)
    {
        ...
        if ($request->ajax()) {
            $data = DcaMaResult::where('ref_id', $ref_id)->select('*');
            return DataTables::of($data)
                ->addColumn('image', function ($row) {
                    $src = config('global.FILE_SERVER_DOMAIN') . $row->img_path . config('global.FILE_SERVER_SECRET_KEY');
                    return "<img src='$src' as='image' rel='preload' width='100' class='dt-img' data-id='" . $row->id . "' style='cursor:pointer' />";
                })
    ...
    public function store_new_task(HttpRequest $request, string $id)
    {
        ...
        return redirect()->route('dca_ma.tasks', $id)->with('success', 'Task created successfully');
    }
    public function manual_labelling(HttpRequest $request, string $id, string $task_id, string $ref_id = null)
    {
        ...
        if (is_null($ref_id)) {
            $ref_id = DcaMaReference::where('task_id', $task_id)->select('id')->orderBy('id', 'asc')->first()->id;
            return redirect()->route('dca_ma.manual_labelling', [$id, $task_id, $ref_id]);
        }
        $application = Application::find($id);
        $page = "manual_labelling"; // blade file in views
        $task = ApplicationTask::with('dataset')->find($task_id);

        $references = DcaMaReference::where('task_id', $task_id)->orderBy('id', 'asc')->pluck('id')->toArray();
        $index = array_search($ref_id, $references);
        if ($index === false) {
            return redirect()->route('dca_ma.tasks', [$id]);
        }

        $next_id = ($index == count($references) - 1) ? null : $references[$index + 1];
        $prev_id = ($index == 0) ? null : $references[$index - 1];

        $reference = DcaMaReference::with(['results'])->find($ref_id);
    ...
    {
        $validation_rules = [
            'id' => 'required|exists:dca_ma_results,id',
        ...
        DB::beginTransaction();
        try {
            $result = DcaMaResult::find($data['id']);
            $result->update([
    ...
    public function report(HttpRequest $request, string $id, string $task_id)
    {
        ...
        $references = DcaMaReference::with('results')->select('*')->where('task_id', $task_id)->get();
